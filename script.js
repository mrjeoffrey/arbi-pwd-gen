...
function generatePassword() {
  ...
  var inputLength = parseInt(prompt("Enter the character length for your password."));
  ...
  var inputLowerCase = confirm(
    "Add lowercase characters? hit 'OK' to include lowercase characters. hit 'Cancel' to not include."
  );
  //  b. uppercase
  var inputUpperCase = confirm(
    "Add UPPERCASE characters? hit 'OK' to include lowercase characters. hit 'Cancel' to not include."
  );
  //  c. numbers
  var inputNumbers = confirm(
    "Add Number characters? hit 'OK' to include lowercase characters. hit 'Cancel' to not include."
  );
  //  d. special characters
  var inputSpecialChar = confirm(
    "Add Special characters? hit 'OK' to include lowercase characters. hit 'Cancel' to not include."
  );
  ...
  while (
    ...
    alert(
      "One parameter must be selected. Try again and select at least 1 parameter."
    );
  ...
  if (inputLowerCase) {
    passwordKey = passwordKey.concat(lowerCase);
  }
  if (inputUpperCase) {
    passwordKey = passwordKey.concat(upperCase);
  }
  if (inputNumbers) {
    passwordKey = passwordKey.concat(numbers);
  }
  if (inputSpecialChar) {
